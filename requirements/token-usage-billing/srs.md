...
**判定規則速查**：以「被捨棄的尾數 (discarded fraction)」與 half-ULP（此處為 0.005）比較。
- `> 0.005` → 進位（任何 mode 同結果）
- `< 0.005` → 捨去（任何 mode 同結果）
- `= 0.005` → **halfway case**，HALF_EVEN 看保留位的奇偶（偶捨、奇進）；HALF_UP 一律進位

| Case | overageTokens | rate | 中間值（BigDecimal，精確） | 被捨尾數 | HALF_UP | **HALF_EVEN（採用）** | 備註 |
|---|---|---|---|---|---|---|---|
| AC4 | 30,000 | 0.0200 | 30 × 0.0200 = 0.6000000000 | 0 | 0.60 | **0.60** | 整除，無捨入 |
| 非 halfway | 12,345 | 0.0150 | 12.345 × 0.0150 = 0.185175 | 0.005175 > 0.005 | 0.19 | **0.19** | 進位（mode-agnostic） |
| 非 halfway | 999 | 0.0200 | 0.999 × 0.0200 = 0.019980 | 0.009980 > 0.005 | 0.02 | **0.02** | 進位（mode-agnostic） |
| 非 halfway | 5 | 0.0150 | 0.005 × 0.0150 = 0.000075 | 0.000075 < 0.005 | 0.00 | **0.00** | 捨去（mode-agnostic） |
| **Halfway，第 2 位偶** | **9,250** | **0.0200** | 9.25 × 0.0200 = **0.18500000** | **= 0.005** | 0.19 | **0.18** | **canonical HALF_EVEN regression fixture**：HALF_EVEN ≠ HALF_UP |
| Halfway，第 2 位奇 | 9,750 | 0.0200 | 9.75 × 0.0200 = 0.19500000 | = 0.005 | 0.20 | **0.20** | 第 2 位「9」奇 → 進位至偶數「20」；HALF_UP 同結果，不可單獨用於 mode 差異驗證 |

> **訂正紀錄 (2026-05-02)**：原表曾列 `(12500, 0.0150) → 0.18`，為錯誤 fixture。`12.5 × 0.0150 = 0.1875` 的被捨尾數為 `0.0075`（> 0.005），**並非 halfway case**，任何捨入模式皆得 `0.19`。已替換為 `(9250, 0.0200) → 0.18`，此為精確 halfway 案例（`0.18500000`），第 2 位「8」為偶數，HALF_EVEN 規則「向偶數捨」→ `0.18`，與 HALF_UP（`0.19`）有差異，可作為驗證 rounding mode 的 regression 案例。
